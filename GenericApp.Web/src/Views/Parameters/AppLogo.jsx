...
const LogoEditor = () => {
    ...
    const [reloadKey, setReloadKey] = useState(+ (new Date()).getMilliseconds());

    const LOGO_URL = useMemo(() =>
        API_BASE_URL + '/img/logo.png?key=' + reloadKey,
    ...
    const handleCancel = () => {
        ...
        setPreviewUrl(null);
        document.getElementById('file-upload-button-app-logo').value = '';
    };
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            const response = await userParametersService.setAppLogo(dataToSend);
        ...
        <Paper elevation={3} sx={{ p: { xs: 2, md: 4 }, margin: '0 auto' ,marginBottom:'20px' }}>
            ...
            <Box sx={{ mt: 3, textAlign: 'center' }}>
                <input
                    accept="image/*"
                    style={{ display: 'none' }}
                    id="file-upload-button-app-logo"
                    type="file"
                    onChange={handleFileChange}
                />

                {!selectedFile && (
                    <label htmlFor="file-upload-button-app-logo">
                        <Button
                            variant="contained"
                            component="span"
                            color="primary"
